      ...
      <v-card-text>
        <div v-if="isAuthenticated && userProfile">
            ...
            <v-list-item v-if="getUserRoles().length > 0">
              <v-list-item-content>
                ...
                <div class="roles-container">
                  <v-chip v-for="role in getUserRoles()" :key="role" small color="primary" class="ma-1">
                    {{ role }}
                  </v-chip>
                </div>
              </v-list-item-content>
          ...
          <div class="authority-section">
            ...
            <v-alert v-if="hasAuthority('HUKUM')" type="info" dense class="mb-2">
              <v-icon left>mdi-gavel</v-icon>
              You have legal authority access
            </v-alert>

            <v-alert v-if="hasAnyAuthority(['ADMIN', 'SDM'])" type="success" dense class="mb-2">
              <v-icon left>mdi-shield-crown</v-icon>
              You have administrative privileges
            </v-alert>

            <v-alert v-if="!hasAnyAuthority(['HUKUM', 'ADMIN', 'SDM', 'ALL'])" type="warning" dense class="mb-2">
